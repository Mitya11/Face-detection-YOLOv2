...
def calc_loc_loss(output ,target ,anchors ,Lobj ,koef = 5):
    ...
    target[:, :, :, :2] = torch.nn.functional.hardtanh(target[:, :, :, :2],0,1)
    Nobj = torch.sum(Lobj)
    for i in range(B):
    ...
    loss *= koef/Nobj
    return loss
def calc_conf_loss(output,target,Lobj,Lnoobj,koef_obj = 1, koef_noobj = 0.3):
    loss = 0
    Nconf = torch.sum(Lobj + Lnoobj*(1-Lobj))/48
    for i in range(B):
        k = torch.reshape(output[0, :, i * 4 + i + 4],(7,7))
        loss += koef_obj* torch.sum((output[:, :, i * 4 + i + 4] - target[:,:,i]*1.4) ** 2 *Lobj[:,:,i])
